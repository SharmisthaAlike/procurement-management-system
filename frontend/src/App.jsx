...
import {
  ...
  ClipboardList, AlertCircle, RefreshCw, CheckCircle2, Zap,
} from 'lucide-react';
...
function App() {
  ...
  const fetchAll = async (isRefresh = false) => {
    ...
    try {
      const [kpiRes, matRes, supRes, ordRes] = await Promise.all([
        fetch(`${API_BASE}/kpis`),
        fetch(`${API_BASE}/materials`),
        fetch(`${API_BASE}/suppliers`),
        fetch(`${API_BASE}/purchase-orders`),
      ]);
      const [kpiData, matData, supData, ordData] = await Promise.all([
        kpiRes.json(), matRes.json(), supRes.json(), ordRes.json(),
      ]);
